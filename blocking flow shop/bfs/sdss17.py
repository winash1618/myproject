# -*- coding: utf-8 -*-
"""
Created on Fri May 29 22:26:35 2020
...
def LocalSearch(pi4,x,y):
    ...
    for i in range(x):
        ...
        if i!=here & i!=0:
            for j in range(y):
                if j==0:
                    if ss2[i]+pj[j]+kk[pi5[i-1],i,j]>d2[i-1,j+1]:
                        print(d3)
                        d3[j]=ss2[i]+pj[j]+kk[pi5[i-1],i,j]
                    else:
                        
                        d3[j]=d2[i-1,j+1]   
                        print(d3)
                elif j<y-1:
                    if d3[j-1]+pj[j]+kk[pi5[i-1],i,j]>d2[i-1,j+1]:
                        d3[j]=d3[j-1]+pj[j]+kk[pi5[i-1],i,j]
    ...
    for j in range(iter):
        ...
        for i in range(ps):
           ...
           # print(pi2)
            #print(a[i,:])
            hip=random.random()
            tom,sd=setuptime(a[i,:],x,y)
            bom,sd=setuptime(pib,x,y)
            hi4[j]=tom
            if hip<pls:
                pi3=LocalSearch(pi2)
                #print(hip)
                tim,sd=setuptime(pi3,x,y)
