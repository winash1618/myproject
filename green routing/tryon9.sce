//here we group depots and customers based on the minimum distance and use it as a initial solution
clc
clear
rokl=1
x=8
...
for y=1:iter
    ...
    if(piio<0.6)
        gg=2
        kop=gg+round((z-2)*rand(pop,(x-1)))
    else
        gg=1
        kop=gg+round((z-1)*rand(pop,(x-1)))
    end
