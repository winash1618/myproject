...
function [mest]=bestisrule(best,bbom,mest)
    for ka=1:pop
        po=zeros(x,z)
        ckom=bbom(ka,:)
        i=1
        ...
        for k=1:x
            for i=1:z
                bii=0.1+rand(1,1)
                if po(k,i+1)~=0
                    if timr(po(k,i),po(k,i+1))>bii
                        r=r+1
                    end
                    if dibr(k,po(k,1))>bii & i==1 
                        r=r+1
                    end
                else
                    if  po(k,i)~=0
                        if po(k,i+1)==0 & dib(k,po(k,i))>bii
                            r=r+1
                        end
                    end
                    break
...
for y=1:iter
    ...
    free=rand(1,1)
    if free<0.05
        [mest]=bestisrule(best,bbom,mest)
    end
    c=best
    bkom=bbom
    dis=dist
