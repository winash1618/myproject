...
function [firo,ciro,n,bkom] = mutate(b,grd,bkom,firo,ciro)
    c=b
    //disp(bkom)
    gnd=ones(pop,x)
    for k=1:pop
    ...
    for k=1:pop
        ...
        for i=1:x
            if grd(k,i)>=1
                if fis1(k,i)<firo(grd(k,i),i)
                    if i==1 
                        for j=1:grd(k,i)
                            ciro(grd(k,i),j,i)=b(k,j)
                            
                        end
                        firo(grd(k,i),i)= fis1(k,i)
                    else
                        u=1
                        for j=gnd(k,i)+1:gnd(k,i)+grd(k,i)
                            ciro(grd(k,i),u,i)=b(k,j)
                            u=u+1
                        end
                        firo(grd(k,i),i)= fis1(k,i)
                    end
    ...
    //disp(p)
    if p==0 then
        [firo,ciro,c,bkom]=mutate(c,grd,bkom,firo,ciro)
    end
    //disp("ddd",b,"dd",c,"df")
    n=c
endfunction
...
for y=1:iter
    ...
    //disp(bkom,gp,a)
    [firo,ciro,a,bkom]=mutate(a,gp,bkom,firo,ciro)
end
plot(ryg,rag)
for ii=1:pop
    sup=best(ii,:)
