        ...
        for k=1:x
            for i=1:z
                ...
                if po1(k,i+1)~=0
                    ...
                    if tim(po1(k,i),po1(k,i+1))>bii & bi1<bi & bi1<bi
                        rep1(ka,mik)=po1(k,i+1)
                    end
                    if dib(k,po1(k,1))>bii & i==1 & bi1<bi
                            rep1(ka,mik)=po1(k,1)
                    end
                    ...
                    if po1(k,i)~=0
                        if po1(k,i+1)==0 & dib(k,po1(k,i))>bii & bi1<bi
                            rep1(ka,mik)=po1(k,i)
                        end
                    end
                    break
...
for y=1:iter
    ...
    for i=1:pop
        ...
        //disp(((1+(exp(-(100/round(sum(bd)/pop))))/2)))
        vol=round(sum(bd)/pop)
        vol1=round(min(bd))
        //vol2=(1+(1.5-((vol-vol1)/20)*1.5))
        bgh=rand(1,1)
        if round(sum(bd)/pop)-round(min(bd))<15 & y>1000 & bgh<0.5
