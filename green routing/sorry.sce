            ...
            if xio<(y/iter)
                ...
                for i=1:x
                    ...
                    if i==1
                        ...
                        mut2=1+round((siz(k,i)-1)*rand(1,1))
                        ...
                        mut2=gnd(k,i)+round((siz(k,i)-1)*rand(1,1))
                        if poll<0.2 & mut1>0 & mut2>0 & mut1~=mut2
                            temper=a(k,mut2)
                            a(k,mut2)=a(k,mut1)
                            a(k,mut1)=temper
...
function [siz]=sizechange(siz)
    for i=1:pop
        ...
        if hit1~=hit2
            if siz(i,hit1)<7 & siz(i,hit2)<7 &  siz(i,hit1)>1
...
for y=1:iter
    ...
    for i=1:pop
        ...
        if round(sum(mog)/pop)-round(min(mog))<50 & y>1000
            if mog1(i)<1.5*mog(i) then
                best(i,:)=c(i,:)
                mog(i)=mog1(i)
                dist(i,:)=dis1(i,:)
                sizb(i,:)=siz1(i,:)
