...
function [keb,rkom,glg,gog] =roulewheel(dis,a,gpd,bbom,bveb)
    tdis=zeros(1,pop)
    pdis=zeros(1,pop)
    cdis=zeros(1,pop)
    calm=a
...
for y=1:iter
    ...
    for i=1:pop
        ...
        if round(sum(bd)/pop)-round(min(bd))<15 & y>1000 & rl>100
            if mog(i)<1.4*min(bd) then
                best(i,:)=a(i,:)
                bd(i)=mog(i)
                gppd(i,:)=gp(i,:) 
                bbom(i,:)=bkom(i,:)
    ...
    a=best
    gpd=gppd
    [veb,bkom,gpd,a]=roulewheel(dis,a,gpd,bkom,veb)
    [a]=mutate(a,gpd)
