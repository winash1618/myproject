...
for i=1:z
    for j=1:z
        ferm(i,j)=min(tim(i,:))/tim(i,j)
    end
end
disp (ferm)
function [kom] =findho(a)
        prob=rand(1,1)
...
function [grp,sis] =finddis(po)
    ...
    mo=zeros(z,z,x)
        ...
        for k=1:x
            ...
            if temp(ka,k)>0
                dis(k)=dis(k)+dib(k,po(k,1))+dib(k,po(k,temp(k)))
            end
        end
    grp=temp
    sis=dis
...
function [glg,gog] =roulewheel(dis,a,gpd)
    tdis=zeros(1,pop)
    pdis=zeros(1,pop)
    cdis=zeros(1,pop)
    calm=a
