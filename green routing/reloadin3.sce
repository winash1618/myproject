...
disp (a)
best=a
function [bkop,grp,sis,fis1] =finddis(a,bkom)
    temp=zeros(pop,x)
    tempo=zeros(pop,x)
    dis=zeros(pop,x)
    dis1=zeros(pop,x)
    ...
    for ka=1:pop
            ...
            for i=1:z
                if i~=25
                    if po(k,i+1)~=0 
                        ...
                        fis(ka,k)=fis(ka,k)+tim(po(k,i),po(k,i+1))*jio(1,k)
                    end
                else
                    break
                end
        ...
        for k=1:x
            for i=1:z
                if i~=25
                    if po1(k,i+1)~=0
                    jio1(1,k)=jio1(1,k)+cap(1,po1(k,i))
                    dis1(ka,k)=dis1(ka,k)+tim(po1(k,i),po1(k,i+1))
                    fis1(ka,k)=fis1(ka,k)+tim(po1(k,i),po1(k,i+1))*jio(1,k)
                    end
               else
                    break
               end
        ...
        for k=1:x
            ...
            if temp(ka,k)>0
                ...
                fis(ka,k)=fis(ka,k)+dib(k,po(k,1))+dib(k,po(k,temp(ka,k)))*jio(1,k)
            end
        end
        for k=1:x
            for i=1:z
            ...
            if tempo(ka,k)>0
                jio1(1,k)=jio1(1,k)+cap(1,po1(k,tempo(ka,k)))
                dis1(ka,k)=dis1(ka,k)+dib(k,po1(k,1))+dib(k,po1(k,tempo(ka,k)))
                fis1(ka,k)=fis1(ka,k)+dib(k,po1(k,1))+dib(k,po1(k,tempo(ka,k)))*jio1(1,k)
...
function [ratio]=findratio(a,gp,dis)
    ratio=zeros(pop,x)
    for ka=1:pop
        po=zeros(x,z)
        sizer=gp(ka,:)
...
for y=1:iter
    //disp(bkom)
    [bkom,gp,dis,fis2]=finddis(a,bkom)
    //[ratio]=findratio(a,gp,dis)
    mog=zeros(1,pop)
    mig=zeros(1,pop)
    mug=ones(1,2*pop)
    mug1=ones(1,pop)
    ...
    rag(y)=min(mog)
    ryg(y)=min(mig)
    [bkom,gp,a]=roulewheel(mug1,a,gp,bkom)
    [a]=mutate(a,gp)
end
plot(ryg,rag)
for ii=1:pop
