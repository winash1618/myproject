...
clear
x=4//no of BMC's
z=15//no of societies
pop=10//population size
iter=30000//max no of iterations
a=zeros(pop,z)//our solution set. 
rag=zeros(iter)//for recording some value every iteration (not important)
//cap is the capacity of each society 
cap=[233	35	53	337	260	161	183	176	86	51	162	79	68	152	111
]
//tim is the distance from societies to societes
...
//dib is  the distance from BMC to societies
...
//next ten line generate initial population
cad=rand(pop,z)
for i=1:pop
    for j=1:z
        for k=1:z
...
//next tem line generate an initial best population
cad=rand(pop,z)
best=a
for i=1:pop
    for j=1:z
...
//next ten lines groups the societies based on their distance from the depot 
kii=zeros(1,x)
for i=1:x
    for j=1:z
        if dib(i,j)<=15
...
disp (a)
function [grp,sis,fis] =finddis(a,bkom)//this function finds distance covered by the vehicle during entire journey also calculates the cumulative carbon emission during the entire journey
    temp=zeros(pop,x)
    dis=zeros(pop,x)
    fis=zeros(pop,x)
    for ka=1:pop
...
function [n,bkom] = mutate(b,grd,bkom)//this function will execute the mutation operation
    gnd=ones(pop,x)
    for k=1:pop
        for i=1:x
            if i==1
...
function [rkom,glg,gog] =roulewheel(dis,a,gpd,bkom)// this function will do the roulette wheel operation
    tdis=zeros(1,2*pop)
    pdis=zeros(1,2*pop)
    cdis=zeros(1,2*pop)
    calm=zeros(pop,z)
...
//Following lines of code display the best route and plot graphs
plot(ryg,rag)
for ii=1:pop
    sup=best(ii,:)
    gd=gpd(ii,:)
