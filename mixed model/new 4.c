...
#define n 15
#define dt 100
int i=0,j=0,k=0,c,sum=0,sum1=0,l=0,m=0,r=0;
float d[n]={15,15,10,10,10,10,10,10,4,1,1,1,1,1,1};
double fit[sn],fit1[sn],roule_prob[sn],cumulativ_prob[sn];
double rand_num,prob=0.5,fitness=0,fitness_new=0;
    ...
    for(i=0;i<n;i++)
        {
            for(j=0;j<dt;j++)
            {
                fit[g]+=((s[i][j]-((j+1)*d[i])/dt)*(s[i][j]-((j+1)*d[i])/dt));
                fit1[g]+=((s1[i][j]-((j+1)*d[i])/dt)*(s1[i][j]-((j+1)*d[i])/dt));
            }
        }
