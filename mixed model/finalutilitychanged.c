...
double fit[sn],fit1[sn],roule_prob[sn],cumulativ_prob[sn],z[sta][dt+1],z1[sta][dt+1],u[sta][dt],u1[sta][dt],mat[sta],mat1[sta];
	...
	for(i=0;i<=dt;i++)
    {
		...
		else
        {
            for(j=0;j<sta;j++)
            {
                ...
                for(k=0;k<n;k++)
				{
					temp+=x[k][dt-1]*tme[j][k];
					tomp+=x1[k][dt-1]*tme[j][k];
					//printf("%f  ",x[k][dt-1]);
				}
