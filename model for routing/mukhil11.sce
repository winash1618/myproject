...
function aa=capmeet(aa)
    ...
    for i=1:v
        ...
        while aa(j)~=0
            casec(i,k)=aa(1,j)
            caper(1,i)=caper(1,i)+cap(1,aa(1,j))
            k=k+1
            j=j+1
        end
        if j<z
    ...
    while calm==0
        for i=1:v
            if caper(1,i)==max(caper) & max(caper)<=maxcap
                calm=1
                break
            elseif caper(1,i)==max(caper)
...
function cs=getobj2()
    ...
    elseif pet1<fi & fi<plt1
        cs=1
    elseif plt<fi & fi<elt1
        cs=
    end
endfunction
function ranker=NonDominatedSorting()
    for i=1:pop
        for j=1:pop
            if obj(1,i)>obj1(1,j) & obj2(1,i)>obj2(1,j)
                ranker(1,i)=ranker(1,i)+3
            elseif obj(1,i)>obj1(1,j) & obj2(1,i)<obj2(1,j)
